  ...
  return (
    <section className={`mb-8 ${className || ''}`}>
      <div className='mb-5 flex items-center justify-between'>
        <div>
          <h2 className='text-2xl font-semibold text-white'>继续观看</h2>
          <p className='text-sm text-white/60 mt-1'>回到上次观看的位置。</p>
        </div>
          ...
          ? // 加载状态显示灰色占位数据
            Array.from({ length: 6 }).map((_, index) => (
              ...
              >
                ...
                <div className='mt-2 h-3 bg-white/10 rounded animate-pulse'></div>
              </div>
            ))
          : // 显示真实数据
